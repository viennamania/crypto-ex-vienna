...
export async function POST(request: NextRequest) {
  try {
    ...
    const rate = evaluateRateLimit({
      key: `api:user:updatePlatformFee:${ipAddress}:${normalizedWalletAddress}`,
      limit: 30,
      windowMs: 60_000,
    });
    if (!rate.allowed) {
    ...
    const signatureAuth = await verifyWalletAuthFromBody({
      body,
      path: '/api/user/updatePlatformFee',
      method: 'POST',
    ...
    const targetUser = await usersCollection.findOne<any>(
      {
        storecode: normalizedStorecode,
    ...
    const prevFee = targetUser?.seller?.platformFee || null;
    ...
    await platformFeeLogsCollection.insertOne({
      walletAddress: String(targetUser?.walletAddress || normalizedWalletAddress),
      storecode: String(targetUser?.storecode || normalizedStorecode || 'admin'),
      prev: prevFee,
      next: nextFee,
      changedAt,
    ...
    console.error('updatePlatformFee failed', error);
