...
import { setPriceSettingMethod } from '@lib/api/user';
...
const ALLOWED_PRICE_SETTING_METHODS = new Set(['fixed', 'market']);
...
export async function POST(request: NextRequest) {
  ...
  const priceSettingMethod = toText(body.priceSettingMethod).toLowerCase();

  if (!ALLOWED_PRICE_SETTING_METHODS.has(priceSettingMethod)) {
    return NextResponse.json(
      {
        error: 'priceSettingMethod is invalid.',
      },
      {
        status: 400,
      },
  ...
  const rate = evaluateRateLimit({
    key: `api:user:setPriceSettingMethod:${ipAddress}:${requestedWalletAddress || 'unknown'}`,
    limit: 30,
    windowMs: 60_000,
  });
  if (!rate.allowed) {
