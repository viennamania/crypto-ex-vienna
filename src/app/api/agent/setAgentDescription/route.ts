...
import { updateAgentDescription } from '@lib/api/agent';
...
export async function POST(request: NextRequest) {
  ...
  const agentDescription = toText(body.agentDescription);
  const requestedWalletAddress = toText(body.requesterWalletAddress || body.walletAddress);

  if (!agentcode || !agentDescription) {
    return NextResponse.json({ error: 'agentcode and agentDescription are required' }, { status: 400 });
  }
  ...
  if (!isAuthorized) {
    return NextResponse.json({ error: 'Only agent admin wallet can update agent description' }, { status: 403 });
  }
