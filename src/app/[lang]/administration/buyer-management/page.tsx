...
export default function BuyerManagementPage() {
  ...
  const [walletBalanceByAddress, setWalletBalanceByAddress] = useState<
    Record<string, BuyerWalletBalanceState>
  >({});
  ...
  const handleCheckBuyerUsdtBalance = async (walletAddress: string) => {
    const normalizedWallet = String(walletAddress || '').trim();
    if (!normalizedWallet) return;
    ...
    try {
      ...
      console.error('Failed to fetch buyer USDT balance', error);
      setWalletBalanceByAddress((prev) => {
        const existing = prev[walletKey];
        return {
          ...prev,
                                ...
                                <div className="flex items-center gap-2">
                                  {cooldownRemainingMs <= 0 ? (
                                    <button
                                      type="button"
                                      onClick={() => {
                                        void handleCheckBuyerUsdtBalance(walletAddress);
                                      }}
