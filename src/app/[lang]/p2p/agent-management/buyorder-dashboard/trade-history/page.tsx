...
import { fetchAgentSummary, type AgentSummary } from '../../_shared';
...
export default function P2PAgentBuyorderTradeHistoryPage() {
  ...
  const buyorderDashboardPath = lang
    ? `/${lang}/p2p/agent-management/buyorder-dashboard${agentQuery}`
    : `/p2p/agent-management/buyorder-dashboard${agentQuery}`;
  const agentWalletAddress = String(activeAccount?.address || '').trim();
  ...
  const orderChatUserId = useMemo(
    () => SENDBIRD_MANAGER_CHAT_USER_ID || agentWalletAddress || String(agent?.adminWalletAddress || '').trim(),
    [agent?.adminWalletAddress, agentWalletAddress],
  );
  const orderChatNickname = useMemo(() => {
    if (SENDBIRD_MANAGER_CHAT_USER_ID) return '관리자';
    const normalizedAgentName = String(agent?.agentName || '').trim();
    if (normalizedAgentName) return normalizedAgentName;
    const walletToken = String(orderChatUserId || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 8);
    return walletToken ? `agent_${walletToken}` : '에이전트';
  }, [agent?.agentName, orderChatUserId]);
  ...
  const fetchTradeHistory = useCallback(async () => {
    ...
    try {
      const [agentSummary, response] = await Promise.all([
        fetchAgentSummary(agentcode),
        fetch('/api/order/getBuyOrderDashboardList', {
          method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(String(payload?.error || '입금확인 거래내역 조회에 실패했습니다.'));
      }
      ...
      const message = String(fetchError?.message || '입금확인 거래내역 조회 중 오류가 발생했습니다.');
      setAgent(null);
      setOrders([]);
      setTotalCount(0);
      setBuyerStoreReferralGroups([]);
  ...
  }, [agentcode, appliedFilters, pageNumber, pageSize]);
  ...
  useEffect(() => {
    ...
    if (!orderChatUserId) {
      setOrderChatSessionToken(null);
      setOrderChatSessionLoading(false);
      setOrderChatSessionError('에이전트 채팅 사용자 ID가 없습니다.');
      return () => {
        cancelled = true;
      };
    }
    ...
    const issueSessionToken = async () => {
      ...
      try {
        ...
        if (!response.ok || !payload?.sessionToken) {
          throw new Error(payload?.error || '채팅 세션 토큰 발급에 실패했습니다.');
        }
        if (!cancelled) {
  ...
  const openOrderChatDrawer = useCallback(async (order: BuyOrderItem, channelUrl: string) => {
    ...
    if (!orderChatUserId) {
      toast.error('에이전트 채팅 사용자 ID가 없습니다.');
      return;
    }
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 pb-20 pt-6 lg:px-6 lg:pt-8">
        <section className="rounded-3xl border border-slate-200/80 bg-white/95 p-5 shadow-[0_28px_60px_-42px_rgba(15,23,42,0.38)] backdrop-blur">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.32em] text-slate-500">Agent Buy Order Trade History</p>
                <h1 className="text-xl font-bold text-slate-900">입금확인 거래내역</h1>
                <p className="text-sm text-slate-500">
                  에이전트 판매관리 하위 페이지입니다. 조회 상태는 입금확인/Private 주문으로 고정됩니다.
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Link
                href={buyorderDashboardPath}
        ...
        {!agentcode ? (
          <section className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
            URL에 `?agentcode=...` 파라미터를 추가해야 입금확인 거래내역을 조회할 수 있습니다.
          </section>
        ) : (
          <>
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white/95 p-4 shadow-[0_18px_38px_-34px_rgba(15,23,42,0.42)]">
              <form className="grid grid-cols-1 gap-3 lg:grid-cols-12" onSubmit={handleSearchSubmit}>
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="inline-flex h-10 w-full items-center rounded-xl border border-cyan-300 bg-cyan-50 px-3 text-sm font-semibold text-cyan-700">
                    {agentcode}
                  </div>
                </div>
                  ...
                  <table className="w-full min-w-[1080px] table-fixed lg:min-w-0">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {orders.map((order, index) => {
                        ...
                        const agentcodeValue = String(order?.agent?.agentcode || order?.agentcode || order?.seller?.agentcode || '').trim();
                            ...
                            <td className="px-3 py-3">
                              <div className="space-y-0.5 text-xs">
                                ...
                                <p className="text-slate-500">
                                  {agentName && agentcodeValue ? `${agentName} (${agentcodeValue})` : (agentName || agentcodeValue || '-')}
                                </p>
                              </div>
                            </td>
