...
export default function Index({ params }: any) {
  ...
  */
  ...
  const signatureAccount = useMemo<SignMessageAccount | null>(() => {
    ...
    return null;
  }, [activeWallet, connectedWallets, rawActiveAccount]);

  const activeAccount = activeWallet?.getAccount?.() ?? rawActiveAccount;
  const address = String(activeAccount?.address || signatureAccount?.address || '').trim();
  ...
  const buildSignedRequestBody = useCallback(
    async ({
      ...
      const auth = await createWalletSignatureAuthPayload({
        ...
        storecode: typeof storecode === 'string' && storecode.trim() ? storecode.trim() : undefined,
        path,
        method,
      });
  ...
  useEffect(() => {
    ...
    const resolveStorecode = (payload: Record<string, unknown>) => {
      ...
      const byQuery = toText(searchParamsStorecode);
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      try {
        ...
        if (!payloadForSign.requesterWalletAddress && address) {
          payloadForSign.requesterWalletAddress = address;
        }
        ...
        console.warn('p2p buy auto-sign fallback', autoSignError);
        return originalFetch(input, init);
      }
    };
  ...
  }, [address, buildSignedRequestBody, searchParamsStorecode, signatureAccount]);
