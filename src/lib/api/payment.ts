      ...
      .aggregate([
        ...basePipeline,
        {
          $project: {
            ...
            platformFeeRate: { $ifNull: ['$platformFeeRate', { $ifNull: ['$platform_fee_rate', 0] }] },
            platformFeeAmount: { $ifNull: ['$platformFeeAmount', { $ifNull: ['$platform_fee_amount', 0] }] },
            platformFeeWalletAddress: {
              $ifNull: ['$platformFeeWalletAddress', { $ifNull: ['$platform_fee_wallet_address', ''] }],
            },
            createdAt: 1,
            confirmedAt: 1,
...
export async function getWalletUsdtPendingOrderProcessingSummaryAll({
  limit = 5,
}: {
  limit?: number;
}): Promise<{
